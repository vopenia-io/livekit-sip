...
func (e *LivekitCompositor) initCamera(self *gst.Bin) error {
	...
	e.LivekitCompositorCamera.FallbackFilter, err = gst.NewElementWithProperties("capsfilter", map[string]interface{}{
		"caps": gst.NewCapsFromString("video/x-raw,width=1280,height=720,framerate=30/1"),
	})
	if err != nil {
		return err
	}

	e.LivekitCompositorCamera.PatchBay, err = gst.NewElementWithProperties("patchbay", map[string]interface{}{})
	if err != nil {
		return err
	}
...
func (e *LivekitCompositor) applyCameraLayout(self *gst.Bin, layout []string) {
	...
	if len(layout) < len(e.currentLayout) {
		for i := len(layout); i < len(e.currentLayout); i++ {
			destPad := e.LivekitCompositorCamera.Compositor.GetStaticPad(fmt.Sprintf("sink_%d", i+1))
			if destPad == nil {
				continue
			}
	...
	for i, participantSID := range layout {
		...
		destPad := e.LivekitCompositorCamera.Compositor.GetStaticPad(fmt.Sprintf("sink_%d", i+1))
		if destPad == nil {
			destPad = e.LivekitCompositorCamera.Compositor.GetRequestPad(fmt.Sprintf("sink_%d", i+1))
			if destPad == nil {
				self.Log(CAT, gst.LevelError, fmt.Sprintf("Failed to get or request sink pad for layout position %d", i))
				self.Error("Failed to get or request sink pad for camera layout", fmt.Errorf("failed to get or request sink pad for camera layout position %d", i))
				return
			}
			if err := destPad.SetProperty("sizing-policy", int(1) /* keep-aspect-ratio */); err != nil {
				self.Log(CAT, gst.LevelWarning, fmt.Sprintf("Failed to set sizing policy on compositor sink pad for layout position %d: %v", i, err))
			}
			srcPad := e.LivekitCompositorCamera.PatchBay.GetRequestPad("src_%u")
			if srcPad == nil {
				self.Log(CAT, gst.LevelError, fmt.Sprintf("Failed to request new source pad for layout position %d", i))
				self.Error("Failed to request new source pad for camera layout", fmt.Errorf("failed to request new source pad for camera layout position %d", i))
				return
			}
			if ret := srcPad.Link(destPad); ret != gst.PadLinkOK {
				self.Log(CAT, gst.LevelError, fmt.Sprintf("Failed to link source pad %s to compositor sink pad %s for layout position %d", srcPad.GetName(), destPad.GetName(), i))
				self.Error("Failed to link camera patchbay source pad to compositor sink pad for layout", fmt.Errorf("failed to link camera patchbay source pad %s to compositor sink pad %s for layout position %d", srcPad.GetName(), destPad.GetName(), i))
				return
			}
		}
		srcPad := destPad.GetPeer()
		if srcPad == nil {
			self.Log(CAT, gst.LevelError, fmt.Sprintf("Compositor sink pad %s for layout position %d is not linked to any source pad", destPad.GetName(), i))
			self.Error("Compositor sink pad for camera layout is not linked to any source pad", fmt.Errorf("compositor sink pad %s for layout position %d is not linked to any source pad", destPad.GetName(), i))
			return
		}
		if _, err := e.LivekitCompositorCamera.PatchBay.Emit("activate-path", sinkPad, srcPad); err != nil {
			self.Log(CAT, gst.LevelError, fmt.Sprintf("Failed to activate path from camera patchbay pad %s to compositor sink pad %s for layout position %d: %v", sinkPad.GetName(), destPad.GetName(), i, err))
			continue
		}
		if err := cameraPadSetPosSize(destPad, i, len(layout)); err != nil {
			self.Log(CAT, gst.LevelWarning, fmt.Sprintf("Failed to set position and size for compositor sink pad %s for layout position %d: %v", destPad.GetName(), i, err))
		}
