...
func TestMain(m *testing.M) {
	glib.SetEnv("GST_DEBUG", glib.GetEnv("GST_DEBUG")+",g711dtmf-audio:5", true)
	gst.Init(nil)
	Register()
	os.Exit(m.Run())
}
...
func TestG711Audio_Pipeline(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			transcoder, err := gst.NewElement("g711dtmf-audio")
			if err != nil {
				t.Fatal("failed to create g711dtmf-audio:", err)
			}
			...
			if count <= 0 {
				t.Fatalf("no buffers received through g711dtmf-audio element (%s path)", tc.name)
			}
		})
	}
}
