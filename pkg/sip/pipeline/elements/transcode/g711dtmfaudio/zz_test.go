...
func TestMain(m *testing.M) {
	glib.SetEnv("GST_DEBUG", glib.GetEnv("GST_DEBUG")+",g711-dtmf-audio:5", true)
...
func TestG711DtmfAudio_Inband(t *testing.T) {
	tests := []struct {
		name      string
		encoder   string
		payloader string
	...
	}{
		{
			...
			dotFile:   "g711_dtmf_audio_pcmu_inband_test.dot",
		},
		{
			name:      "PCMA/Inband",
			encoder:   "alawenc",
			payloader: "rtppcmapay",
			dotFile:   "g711_dtmf_audio_pcma_inband_test.dot",
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			pipeline, err := gst.NewPipeline(fmt.Sprintf("test-g711-dtmf-audio-inband-%s", tc.name))
			if err != nil {
			...
			transcoder, err := gst.NewElement("g711-dtmf-audio")
			if err != nil {
				t.Fatal("failed to create g711-dtmf-audio:", err)
			}
			...
			if count <= 0 {
				t.Fatalf("no buffers received through g711-dtmf-audio element (%s path)", tc.name)
			}
...
func TestG711DtmfAudio_Outofband(t *testing.T) {
	tests := []struct {
		name      string
		encoder   string
		payloader string
	...
	}{
		{
			...
			encoder:   "mulawenc",
			payloader: "rtppcmupay",
			dotFile:   "g711_dtmf_audio_pcmu_outofband_test.dot",
		},
		{
			name:      "PCMA/Outofband",
			encoder:   "alawenc",
			payloader: "rtppcmapay",
			dotFile:   "g711_dtmf_audio_pcma_outofband_test.dot",
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			pipeline, err := gst.NewPipeline(fmt.Sprintf("test-g711-dtmf-audio-outofband-%s", tc.name))
			if err != nil {
				t.Fatal("failed to create pipeline:", err)
			}
