...
func TestMain(m *testing.M) {
	glib.SetEnv("GST_DEBUG", glib.GetEnv("GST_DEBUG")+",opus-audio:5", true)
	gst.Init(nil)
	Register()
	os.Exit(m.Run())
}

func TestOpusAudio_Pipeline(t *testing.T) {
	defer testutils.AssertNoLeaks(t)

	pipeline, err := gst.NewPipeline("test-opus-audio")
	if err != nil {
	...
	payloader, err := gst.NewElement("rtpopuspay")
	if err != nil {
		t.Fatal("failed to create rtpopuspay:", err)
	}

	transcoder, err := gst.NewElement("opus-audio")
	if err != nil {
	...
	if err := os.WriteFile("opus_audio_test.dot", []byte(dotData), 0644); err != nil {
	...
	if count <= 0 {
		t.Fatal("no buffers received through opus-audio element")
	}
}
