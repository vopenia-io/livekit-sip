...
func TestVp9Video_Smoke(t *testing.T) {
	...
	pipeline, err := gst.NewPipeline("vp9video-smoke")
	if err != nil {
	...
	if got := bufferCount.Load(); got <= 0 {
		t.Fatalf("no buffers received through vp9-video; expected > 0, got %d", got)
	}
}
