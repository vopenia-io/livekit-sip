...
// Exported test helpers for video-av1. Must not import "testing" or
// introduce globals/init — otherwise DCE can't strip these symbols
// from production binaries that import videoav1.
...
func (TestElement) Name() string { return "video-av1" }
...
func (TestElement) BuildElement(p *gst.Pipeline, targetWidth, targetHeight int) (*gst.Element, error) {
	e, err := gst.NewElementWithProperties("video-av1", map[string]any{
		"video-width":  uint(targetWidth),
		"video-height": uint(targetHeight),
	})
	if err != nil {
		return nil, fmt.Errorf("video-av1: %w", err)
	}
	if err := p.Add(e); err != nil {
		return nil, fmt.Errorf("add video-av1: %w", err)
	}
	return e, nil
}
