...
func TestMain(m *testing.M) {
	glib.SetEnv("GST_DEBUG", glib.GetEnv("GST_DEBUG")+",audio-g722:5", true)
	gst.Init(nil)
	Register()
	os.Exit(m.Run())
}

func TestAudioG722_Pipeline(t *testing.T) {
	defer testutils.AssertNoLeaks(t)

	pipeline, err := gst.NewPipeline("test-audio-g722")
	if err != nil {
	...
	transcoder, err := gst.NewElement("audio-g722")
	if err != nil {
		t.Fatal("failed to create audio-g722:", err)
	}
	...
	if err := os.WriteFile("audio_g722_test.dot", []byte(dotData), 0644); err != nil {
	...
	if count <= 0 {
		t.Fatal("no buffers received through audio-g722 element")
	}
}
