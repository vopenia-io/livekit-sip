...
func TestH264Video_Smoke(t *testing.T) {
	...
	pipeline, err := gst.NewPipeline("h264video-smoke")
	if err != nil {
	...
	if got := bufferCount.Load(); got <= 0 {
		t.Fatalf("no buffers received through h264-video; expected > 0, got %d", got)
	}
}
