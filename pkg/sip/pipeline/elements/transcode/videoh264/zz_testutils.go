...
// Exported test helpers for video-h264. Must not import "testing" or
// introduce globals/init — otherwise DCE can't strip these symbols
// from production binaries that import videoh264.
...
func (TestElement) Name() string { return "video-h264" }
...
func (TestElement) BuildSource(p *gst.Pipeline, width, height, fps, numBuffers int) (*gst.Pad, error) {
	...
	return caps.GetStaticPad("src"), nil
}

func (TestElement) BuildElement(p *gst.Pipeline, targetWidth, targetHeight int) (*gst.Element, error) {
	e, err := gst.NewElementWithProperties("video-h264", map[string]any{
		"video-width":  uint(targetWidth),
		"video-height": uint(targetHeight),
	})
	if err != nil {
		return nil, fmt.Errorf("video-h264: %w", err)
	}
	if err := p.Add(e); err != nil {
		return nil, fmt.Errorf("add video-h264: %w", err)
	}
	return e, nil
}
