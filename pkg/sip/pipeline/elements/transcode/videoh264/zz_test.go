...
func TestVideoH264_Pipeline(t *testing.T) {
	defer testutils.AssertNoLeaks(t)

	pipeline, err := gst.NewPipeline("test-video-h264")
	if err != nil {
	...
	transcoder, err := gst.NewElement("video-h264")
	if err != nil {
		t.Fatal("failed to create video-h264:", err)
	}
	...
	if err := os.WriteFile("video_h264_test.dot", []byte(dotData), 0644); err != nil {
	...
	if count <= 0 {
		t.Fatal("no buffers received through video-h264 element")
	}
}
