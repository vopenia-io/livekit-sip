...
func TestNvVideoAv1_Smoke(t *testing.T) {
	...
	pipeline, err := gst.NewPipeline("nvvideoav1-smoke")
	if err != nil {
	...
	if got := bufferCount.Load(); got <= 0 {
		t.Fatalf("no buffers received through nv-video-av1; expected > 0, got %d", got)
	}
}
