...
func TestNvH264Video_Smoke(t *testing.T) {
	...
	pipeline, err := gst.NewPipeline("nvh264video-smoke")
	if err != nil {
	...
	if got := bufferCount.Load(); got <= 0 {
		t.Fatalf("no buffers received through nv-h264-video; expected > 0, got %d", got)
	}
}
