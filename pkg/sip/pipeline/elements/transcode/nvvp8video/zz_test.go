...
func TestNvVp8Video_Smoke(t *testing.T) {
	...
	pipeline, err := gst.NewPipeline("nvvp8video-smoke")
	if err != nil {
	...
	if got := bufferCount.Load(); got <= 0 {
		t.Fatalf("no buffers received through nv-vp8-video; expected > 0, got %d", got)
	}
}
