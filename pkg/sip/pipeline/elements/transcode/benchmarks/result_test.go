...
//
// All measurement lives in main:
//   * Residency — pad probes bracketing the two unixfd hops (so it
//     includes the round-trip IPC tax, but that tax is roughly
//     constant across elements so comparisons stay meaningful).
...
//
// The child just runs the EUT; parsing and stats live on the main
// side so the measurement window captures only actual EUT work.
type Result struct {
