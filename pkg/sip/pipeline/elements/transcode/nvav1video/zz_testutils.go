...
// Exported test helpers for nv-av1-video. Must not import "testing"
// or introduce globals/init — otherwise DCE can't strip these symbols
// from production binaries that import nvav1video.
...
func (TestElement) Name() string { return "nv-av1-video" }

// BuildSource: videotestsrc -> capsfilter -> svtav1enc -> av1parse ->
// rtpav1pay. Identical to the CPU av1-video source chain — the GPU
// element's sink pad consumes RTP so no cudaupload is needed here.
// Keeping the source on the CPU ensures we measure only the GPU
// element's cost, not shared GPU compute from the sample generator.
...
func (TestElement) BuildElement(p *gst.Pipeline, targetWidth, targetHeight int) (*gst.Element, error) {
	e, err := gst.NewElementWithProperties("nv-av1-video", map[string]any{
		"video-width":  uint(targetWidth),
		"video-height": uint(targetHeight),
	})
	if err != nil {
		return nil, fmt.Errorf("nv-av1-video: %w", err)
	}
	if err := p.Add(e); err != nil {
		return nil, fmt.Errorf("add nv-av1-video: %w", err)
	}
	return e, nil
}
