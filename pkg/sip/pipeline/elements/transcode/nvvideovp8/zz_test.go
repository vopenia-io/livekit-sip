...
func TestNvVideoVp8_Smoke(t *testing.T) {
	...
	pipeline, err := gst.NewPipeline("nvvideovp8-smoke")
	if err != nil {
	...
	if got := bufferCount.Load(); got <= 0 {
		t.Fatalf("no buffers received through nv-video-vp8; expected > 0, got %d", got)
	}
}
