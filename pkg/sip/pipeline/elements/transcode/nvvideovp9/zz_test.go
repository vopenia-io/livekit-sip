...
func TestNvVideoVp9_Smoke(t *testing.T) {
	...
	pipeline, err := gst.NewPipeline("nvvideovp9-smoke")
	if err != nil {
	...
	if got := bufferCount.Load(); got <= 0 {
		t.Fatalf("no buffers received through nv-video-vp9; expected > 0, got %d", got)
	}
}
