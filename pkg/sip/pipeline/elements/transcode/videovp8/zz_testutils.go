...
// Exported test helpers for video-vp8. Must not import "testing" or
// introduce globals/init — otherwise DCE can't strip these symbols
// from production binaries that import videovp8.
...
func (TestElement) Name() string { return "video-vp8" }
...
// needs to produce raw video — no encoder on this side. Emits raw
// video in system memory (memType 0).
...
func (TestElement) BuildElement(p *gst.Pipeline, targetWidth, targetHeight int) (*gst.Element, error) {
	e, err := gst.NewElementWithProperties("video-vp8", map[string]any{
		"video-width":  uint(targetWidth),
		"video-height": uint(targetHeight),
	})
	if err != nil {
		return nil, fmt.Errorf("video-vp8: %w", err)
	}
	if err := p.Add(e); err != nil {
		return nil, fmt.Errorf("add video-vp8: %w", err)
	}
	return e, nil
}

// BuildSink: fakesink. Encoder output is RTP (system memory, memType 0).
