...
func TestMain(m *testing.M) {
	glib.SetEnv("GST_DEBUG", glib.GetEnv("GST_DEBUG")+",video-vp8:5", true)
	gst.Init(nil)
	Register()
	os.Exit(m.Run())
}

func TestVideoVp8_Pipeline(t *testing.T) {
	defer testutils.AssertNoLeaks(t)

	pipeline, err := gst.NewPipeline("test-video-vp8")
	if err != nil {
	...
	transcoder, err := gst.NewElement("video-vp8")
	if err != nil {
	...
	if err := pipeline.AddMany(videoSrc, capsFilter, transcoder, sink); err != nil {
		t.Fatal("failed to add elements to pipeline:", err)
	}

	if err := gst.ElementLinkMany(videoSrc, capsFilter, transcoder, sink); err != nil {
	...
	if err := os.WriteFile("video_vp8_test.dot", []byte(dotData), 0644); err != nil {
	...
	if count <= 0 {
		t.Fatal("no buffers received through video-vp8 element")
	}
}
