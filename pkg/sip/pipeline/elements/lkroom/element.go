...
func onRoomJoin(s *lkroom, self *gst.Bin) bool {
	if s == nil {
		self.Log(CAT, gst.LevelError, "lkroom instance has been freed")
		self.ErrorMessage(gst.DomainResource, gst.ResourceErrorSettings, "lkroom instance has been freed in onRoomJoin", "")
		return false
	}
	self.Log(CAT, gst.LevelDebug, "Emitted join-room signal")
	...
	case gst.StatePlaying, gst.StatePaused, gst.StateReady:
		self.Log(CAT, gst.LevelInfo, "Element is already in PLAYING/PAUSED/READY state, joining room now")
		err := s.joinRoom(self)
		if err != nil {
			self.Log(CAT, gst.LevelError, fmt.Sprintf("Error connecting to room: %v", err))
			self.ErrorMessage(gst.DomainResource, gst.ResourceErrorSettings, "Error connecting to room", err.Error())
		...
		return true
	default:
		self.Log(CAT, gst.LevelInfo, "Element is not in PLAYING/PAUSED/READY state, emitting join-room signal to trigger join on state change")
		self.Emit("join-room")
		return false
	}
}
...
func (s *lkroom) start(self *gst.Bin) gst.StateChangeReturn {
	...
	if s.AutoJoin {
		...
		callbackRet := make(chan bool)
		go func() {
			<-callback
			defer self.Element.UnlockState()
			// defer bin.HandlerDisconnect(hnd)
			if onRoomJoin(s, self) {
